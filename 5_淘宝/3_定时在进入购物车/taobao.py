    ...
    def buy_on_time(self):
             while True:
               ...
               if now.strftime('%Y-%m-%d %H:%M:%S') == self.buytime:
                 while True:
                   try:
                     self.browser.get("https://cart.taobao.com/cart.htm")     #购物车.放在这个地方相当于刷新，不然可能不能点击全选
                     sleep(0.1)
                     self.browser.find_element_by_id("J_SelectAll1").click()  #购物车全选
                     sleep(0.1)
...
buytime = '2019-04-20 11:58:00'   #下单时间
