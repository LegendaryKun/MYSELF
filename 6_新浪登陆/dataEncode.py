...
def encode_username(username):
    #return base64.encodestring(username)[:-1]  #对字符串编码  #//2.7中参数直接是字符串，3.几只能是二进制
    return base64.encodestring(bytes(username, encoding="utf8"))[:-1]  #//[:-1]删除结尾字符
    
#使用rsa2对password进行编码
def encode_password(password, servertime, nonce, pubkey):
    rsaPubkey = int(pubkey, 16)  #//指定pubkey为十六进制，输出为10进制,pubkey为字符串
    RSAKey = rsa.PublicKey(rsaPubkey, 65537) #创建公钥  //根据博主分析可知
    codeStr = str(servertime) + '\t' + str(nonce) + '\n' + str(password) #根据js拼接方式构造明文
    #pwd = rsa.encrypt(codeStr, RSAKey)  #使用rsa进行加密 //codeStr必须使字节
    pwd = rsa.encrypt(bytes(codeStr,encoding="utf8"), RSAKey)
    return binascii.b2a_hex(pwd)  #将加密信息转换为16进制。//可以在network中看到密码sp是由十六进制组成的
...
def get_prelogin_info():
    ...
    data = json.loads(jsonStr)  #将已编码的 JSON 字符串解码为 Python 对象,其实就是dict
